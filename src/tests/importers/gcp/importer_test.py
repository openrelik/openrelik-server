...
def test_download_file_from_gcs(importer_lib, mocker):
    ...
    download_file_from_gcs(mock_storage_client, "my-bucket", "my-object", "/path/to/output")
...
def test_process_gcs_message_success(importer_lib, mocker):
    ...
    mock_message.data = b'{"bucket": "my-bucket", "name": "folder/file.txt", "size": "100"}'
    
    mock_db = mocker.MagicMock()
    
    valid_uuid_str = "12345678-1234-5678-1234-567812345678"
    mock_extract_file_info.return_value = (1, "file.txt", ".txt", f"{valid_uuid_str}.txt")
...
def test_main_success(importer_lib, mocker):
    _, _, main = importer_lib
    mock_subscriber_client = mocker.patch("importers.gcp.importer.pubsub_v1.SubscriberClient")
