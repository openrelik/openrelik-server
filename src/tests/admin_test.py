...
def test_create_api_key_success(mocker):
    ...
    mocker.patch(
        "admin.get_config",
        return_value={"auth": {"jwt_header_default_refresh_expire_minutes": 60}},
    )
    mocker.patch("admin.create_jwt_token", return_value="fake_refresh_token")
    mocker.patch(
        "admin.validate_jwt_token", return_value={"jti": "fake_jti", "exp": 1234567890}
    )
...
def test_purge_deleted_files_success(mocker):
    ...
    mocker.patch(
        "admin.get_config", return_value={"server": {"storage_path": "/tmp/storage"}}
    )
    ...
    mock_partition = [
        (1, mocker.MagicMock(hex="file_uuid"), "txt", 10)
    ]  # file_id, uuid, ext, folder_id
    ...
    mock_execute.side_effect = [
        ...
        mocker.MagicMock(
            partitions=mocker.MagicMock(return_value=[mock_partition])
        ),  # 2. Streaming files
        mocker.MagicMock(
            all=mocker.MagicMock(
                return_value=[(10, mocker.MagicMock(hex="folder_uuid"))]
            )
        ),  # 3. Folder components
...
def test_delete_workflow_template_not_found(mocker):
    ...
    mocker.patch(
        "admin.delete_workflow_template_from_db",
        side_effect=ValueError("Workflow template with ID 1 not found."),
    )
...
def test_create_group_success(mocker):
    ...
    result = runner.invoke(
        app, ["create-group", "newgroup", "--description", "New Group"]
    )
