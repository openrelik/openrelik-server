...
def generate_sql_summary(llm_provider: str, llm_model: str, file_id: int, file_summary_id: int):
    """Generate a summary for a given filebased SQL database.
    ...
    SYSTEM_INSTRUCTION = """
    ### **Database Analysis**
    **Task:** Analyze the provided database schema and sample data.
    **Goal:** Provide a concise summary of the database's structure and content, with a specific focus on identifying any interesting or significant data that could be relevant to understanding system activity.

    ### **Output Format:**
    * Markdown with clear subtitles.
    * Use bold for important terms.
    * Do not add a title for the summary.
    """
    ...
    for table in tables_schemas.keys():
        query = f"SELECT * FROM {table} USING SAMPLE 1 ROWS;"
        try:
            result = duckdb_utils.run_query(file, query)
    ...
    try:
        summary = llm.generate(
            prompt=PROMPT.format(tables_schemas=tables_schemas, sample_data=sample_data)
        )
    # Broad exception handling because we don't know what the AI provider will raise.
    except Exception as e:
        summary = f"There was a problem with the analysis: {e}"
